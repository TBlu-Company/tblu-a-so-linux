...
const linuxDiskIO = require('./actions/linuxDiskIO')
const linuxSysctl = require('./actions/linuxSysctl')
const mLinuxPS = require('./actions/mLinuxPS')
...
exports.run = (data, dBconfig) => new Promise((resolve, reject) => {
  switch (data.moduleFunction) {
    ...
    case "mLinuxCPUTemperature":
      ...
      break;
    case "mLinuxDiskIO":
      linuxDiskIO(data, dBconfig).then(result => resolve(result)).catch(error => reject(error));
      break;
    case "mLinuxSysctl":
      linuxSysctl(data, dBconfig).then(result => resolve(result)).catch(error => reject(error));
      break;
    case "mLinuxPS":
      mLinuxPS(data, dBconfig).then(result => resolve(result)).catch(error => reject(error));
      break;
    default:
      reject(new Error('unknow funcition: ' + data.moduleFunctio));
      break;
  }
});
